<script setup>
// Support both `v-model:number` (prop `number`) and default `v-model` (prop `modelValue`).
...
// Local state for smooth typing and slider interaction
...
function onNumberInput(e) {
  ...
  // Allow immediate UI updates without emitting until commit
  const n = Number(raw)
...
function onRangeInput(e) {
  ...
  local.value = n
  // Light debounce to avoid recomputing charts on every pixel move
...
<template>
  ...
  <input v-model.number="local" class="input w-28" type="number" @change="onNumberChange" @input="onNumberInput">
